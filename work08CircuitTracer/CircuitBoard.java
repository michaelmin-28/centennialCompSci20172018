...
public class CircuitBoard {
	...
	public CircuitBoard(String filename) throws FileNotFoundException, InvalidFileFormatException {
		...
		int fakeCols = 0;
		try {
			fakeRows = fileScan.nextInt();
		}
		catch(InputMismatchException e) {
			InvalidFileFormatException p = new InvalidFileFormatException(filename);
			throw(p);
		}
		...
		while (fileScan.hasNextLine()) {
			colCounter = 0;
			String stringy = fileScan.nextLine();
			String[] parts = stringy.split("\\s");
			for (int j = 0; j < parts.length; j++) {
				if (parts[j].length() != 0) {
					char party = parts[j].charAt(0);
					if (rowCounter < ROWS && j < COLS) {
						board[rowCounter][j] = party;
					}
					colCounter++;
				}	
			}
		...
		for (int i = 0; i < ROWS; i++) {
			...
			for (int j = 0; j < COLS; j++) {
				...
				if (ALLOWED_CHARS.indexOf(board[i][j]) < 0) {
					allowed = false;
				}
				numVals++;
			}
		}
		if (numVals != ROWS * COLS || oneCounter != 1 || twoCounter != 1 || !allowed || !valid) {
